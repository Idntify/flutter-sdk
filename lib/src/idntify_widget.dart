...
class Idntify extends StatefulWidget {
  ...
  Idntify(this.apiKey, this.origin, this.cameras, {Key key, this.stage, this.onStepChange, this.onTransactionFinished}) : super(key: key);
...
class _IdnitfyState extends State<Idntify> {
    ...
    if (widget.cameras.length > 0) {
      ...
      if (_frontCamera != null || _backCamera != null) {
        if (flip && _cameraController != null) {
          ...
          cameraToSet = currentCamera == CameraLensDirection.front ? CameraLensDirection.back : CameraLensDirection.front;
        }
  
        _cameraController = CameraController(cameraToSet == CameraLensDirection.front ? _frontCamera ?? _backCamera : _backCamera ?? _frontCamera, ResolutionPreset.high);
  ...
  Widget build(BuildContext context) {
    switch (currentStep) {
            ...
            builder: (contextFutureBuilder, snapshot) {
              ...
              return Info(
                  ...
                  texts: [
                    ...
                    InfoText('Usa las cámaras de tu teléfono para tomar una foto de:', padding: 10),
      ...
      case 2:
        var texts = [
          InfoText('Usa las cámaras de tu teléfono para tomar una foto por cada lado de tu INE.', padding: 5),
          if (!_loadFiles) ...{
            InfoText('Requerimos de:', bold: true, padding: 5),
              InfoText('1 foto de la parte frontal', icon: TextIcon.front, padding: 10),
              InfoText('1 foto de la parte posterior', icon: TextIcon.reverse, padding: 10),
          }
        ];
        List<Button> buttons = [
          if (!_loadFiles) ...{
        ...
        if (_loadFiles) {
          ...
          if (_frontalID != null && _reverseID == null && _loadedImage) {
            Timer(Duration(seconds: 1), () => setState(() => {_loadedImage = false, _frontalIDLoaded = true}));
          }
        ...
        _widgeToRender = Info(
            ...
            texts: texts,
            imagePicker: _loadFiles ? ImagePickerSelector(
                imageToLoad, 
                _loadingImage ? ImagePickerIcon.loading : _loadedImage ? ImagePickerIcon.loaded : ImagePickerIcon.load, 
                ...
                onTap: () async {
                  try {
                    ...
                    setState(() {
                      _frontalID != null ?
                        _reverseID = image : _frontalID = image;
                      _loadingImage = true;
                    });

                    await setImage(image, _apiService, _frontalID != null && _reverseID == null ? DocumentType.frontal: DocumentType.back);
      ...
      case 5:
        if (!_takePicture) {
          ...
          _widgeToRender = Instructions(
              _frontalIDLoaded ? 'Captura el reverso de tu INE' : 'Captura el frente de tu INE',
              _frontalIDLoaded ? InstructionImage.reverse : InstructionImage.front);
        } else {
          _widgeToRender = Camera(
              _cameraController,
              changeCameraOption: _hasBothCameras,
              text: _frontalIDLoaded ? 'Toma una foto del reverso de tu INE' : 'Toma una foto del frente de tu INE',
      ...
      case 6:
        _widgeToRender = Cropper(
            ...
            onContinue: () async {
              ...
              await _apiService.addDocument(result, _frontalID != null && _reverseID == null ? DocumentType.frontal: DocumentType.back);
      ...
      case 7:
        _widgeToRender = Info(
            ...
            texts: [
              InfoText('Compararemos la captura de tu rostro con la fotografía de tu identificación', padding: 5),
              InfoText('Instrucciones generales:', bold: true, padding: 5),
              InfoText('Muestra el frente de tu rostro, asegurate de que tus ojos se vean claramente y sonríe', icon: TextIcon.first, padding: 10),
              InfoText('Evita usar lentes, audifonos o algún otro accesorio en tu cabeza', icon: TextIcon.second, padding: 10),
            ],
            imagePicker: null,
            buttons: [
              Button('Continuar', onPressed: () {
      ...
      case 8:
        if (!_takePicture) {
          _showLogo = false;
          Timer(Duration(seconds: 4), () => getCameras(cameraToSet: CameraLensDirection.front));
          _widgeToRender = Instructions(
              'Captura de selfie',
              InstructionImage.selfie);
        } else {
          _widgeToRender = Camera(
              _cameraController,
      ...
      case 9:
        ...
        _widgeToRender = Info(
            ...
            texts: [
              InfoText('La verificación ha sido completada exitosamente')
            ],
            imagePicker: null,
            buttons: [],
        );
